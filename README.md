...
Each variation comes with a **Summary Card** (direction, palette, fonts, layout, signature detail) so you can compare at a glance without reading code. Actions are grouped into **Reshape / Tune / Refine / Export** categories to reduce choice overload.
...
每个变体附带**摘要卡片**（方向、色板、字体、布局、签名细节），一眼就能比较三个方向，无需读代码。操作按 **Reshape / Tune / Refine / Export** 四组分类，减少选择困难。
